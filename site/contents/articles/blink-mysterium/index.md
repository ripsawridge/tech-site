...
Drat, something is wrong in that output...let me try a different approach.
...
If we use an IC in optimized code it *means we've given up trying to optimize it*. The code is
...
I changed the code because I wanted to see the KeyedLoadIC go away. My change made the keyed load
run in polymorphic 2-degree state. Why wasn't this enough? Time to debug TurboFan. We have code
...
an indexed interceptor on one of the maps. I waddled over to Toon's desk (they feed us a lot here,
...
Moving on to the line `id = elem[ internalKey ];`. `internalKey` is a string like "jQuery1910384933".
The receiver is a `JS_API_OBJECT_TYPE`.
...
Wow, this is a new and exotic thing for me. An Api object...a comment in `objects.h` sez
"Like JS_OBJECT_TYPE but created from Api functions." God forbid it would be `JS_SPECIAL_API_OBJECT_TYPE`
because that would mean it needs security checks, or has named or indexed property accessors.

Anyway, in the IC system we don't find the property on the object, so we prepare for a non-existent
load. We create a "Tuple3" which is an array with three values: a holder cell, a smi-handler and a validity cell.
...
You might think -- hey, why does this IC go from monomorphic right to megamorphic state, skipping polymorphic?
Because it's a keyed load with a string key and not a normal property load.

So this load is quite complicated. Megamorphic state because of the different maps. Looks like we usually
try to load a non-existent property (at least the cases I saw), and even that requires quite a bit of
